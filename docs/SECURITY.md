...
This pass audited the RLS migrations (`supabase/migrations/001`–`007`), the four
...
| S-14 | RLS / alerts | low | The `alerts` owner-update policy (004) lets owners update **any** column of their own alerts (e.g. `price_original`), not just `user_status`. `with check (auth.uid() = user_id)` does prevent reassigning `user_id`. | **TODO** — column-level restriction is not expressible in plain RLS. If alert integrity ever matters (analytics, billing), add a `BEFORE UPDATE` trigger rejecting changes to non-`user_status` columns, or revoke column UPDATE privileges from `authenticated`. |
...
**Counts:** high 1 · medium 7 · low 6 · info 3 — **FIXED 6 · ACCEPTED 8 · TODO 3**.
...
- **RLS is enabled on every table** (`profiles`, `searches`, `alerts`,
  `audit_logs`, `feedback`) and every owner policy keys on `auth.uid()`.
  No policy allows cross-user reads or writes. Update policies either declare
  `with check (auth.uid() = user_id)` (alerts) or inherit the `USING` clause as
  the check (profiles, searches) — in both cases a row cannot be reassigned to
  another user.
